...
DELIMITER $$
USE `pc_workshop`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pc_workshop`.`orders_BEFORE_DELETE` BEFORE DELETE ON `orders` FOR EACH ROW
BEGIN
...
USE `pc_workshop`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pc_workshop`.`pc_accessories_AFTER_INSERT` AFTER INSERT ON `pc_accessories` FOR EACH ROW
BEGIN
	SET @pcid = NEW.pc_id;
	SET @accessoryid = NEW.accessory_id;
    
    set @price = (SELECT price FROM accessories WHERE accessory_id like @accessoryid);  
    
    update pc set pc.total_price = (pc.total_price + @price) WHERE pc.pc_id like @pcid;
  
END$$


USE `pc_workshop`$$
CREATE DEFINER = CURRENT_USER TRIGGER `pc_workshop`.`pc_accessories_BEFORE_DELETE` BEFORE DELETE ON `pc_accessories` FOR EACH ROW
BEGIN
	SET @pcid = OLD.pc_id;
...
BEGIN
SELECT * FROM employees LEFT OUTER JOIN positions ON positions.position_id = employees.position_id;
END$$
DELIMITER ;
