...
-- DROP TRIGGER IF EXISTS `pc_workshop`.`orders_BEFORE_INSERT`;
-- DELIMITER $$
-- USE `pc_workshop`$$
-- CREATE DEFINER = CURRENT_USER TRIGGER `pc_workshop`.`orders_BEFORE_INSERT` AFTER INSERT ON `orders` FOR EACH ROW
-- BEGIN
-- insert into orders SET NEW.order_id = order_id + 1000;
-- END$$
-- DELIMITER ;orders_BEFORE_INSERT